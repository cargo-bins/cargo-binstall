...
"./$1" binstall --no-confirm cargo-binstall@0.11.1 | tee /dev/stderr | grep -q 'cargo-binstall v0.11.1 is already installed'

"./$1" binstall --no-confirm cargo-binstall@0.10.0 | tee /dev/stderr| grep -q -v 'cargo-binstall v0.10.0 is already installed'

## Test When 0.11.0 is installed but can be upgraded.
"./$1" binstall --no-confirm cargo-binstall@0.12.0
"./$1" binstall --no-confirm cargo-binstall@0.12.0 | tee /dev/stderr| grep -q 'cargo-binstall v0.12.0 is already installed'
"./$1" binstall --no-confirm cargo-binstall@^0.12.0 | tee /dev/stderr| grep -q -v 'cargo-binstall v0.12.0 is already installed'
